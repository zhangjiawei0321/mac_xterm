...
final class AppModel: ObservableObject {
    ...
    // MARK: 会话日志缓存上限（仅影响「保存之前的日志」）
    /// 日志缓存上限（MB，0=不限），持久化；默认 10MB。
    /// 只在“保存当前日志（保存之前的日志）”那一刻生效；屏幕打印与“保存接下来的日志”不受限。
    var logCacheMB: Int {
        get {
...
extension AppModel {
    ...
    /// 保存「当前日志」（保存之前的日志）。
    /// 若超过缓存上限，在保存那一刻再询问怎么保存（打印与「保存接下来的日志」不受此限）。
    func saveLog(_ tab: TerminalTab? = nil) {
        guard let c = (tab ?? selectedTab)?.controller,
              let exported = c.exportLogData(timestamped: logTimestamped) else { return }
        var data = exported
        let capBytes = logCacheMB * 1024 * 1024
        if logCacheMB > 0, data.count > capBytes {
            let totalMB = Double(data.count) / 1_048_576
            let alert = NSAlert()
            alert.messageText = "日志超过缓存上限"
            alert.informativeText = String(format: "本次保存的日志约 %.1f MB，超过上限 %d MB。\n选择保存方式：", totalMB, logCacheMB)
            alert.addButton(withTitle: "全部保存")
            alert.addButton(withTitle: "只保存最近 \(logCacheMB) MB")
            alert.addButton(withTitle: "取消")
            switch alert.runModal() {
            case .alertSecondButtonReturn:
                data = Data("（日志超过上限 \(logCacheMB) MB，仅保存最近 \(logCacheMB) MB）\n".utf8) + data.suffix(capBytes)
