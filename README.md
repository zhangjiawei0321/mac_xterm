...
1. 左侧功能条点 **SSH** 或按 ⌘N → 填主机/端口/用户名 → **「密码」填上保存的密码**（可留空）→ “保存后立即连接”。
2. 连接流程：
   - **保存了密码**：应用通过 `SSH_ASKPASS` 自动登录，终端里直接进入 shell，无需再输密码；
   - **密码留空**：在终端里交互输入（首次连接的指纹已用 `accept-new` 自动接受）。
3. 通过 `exit` 退出时，标签标题会加上“（已断开）”，点 × 关闭即可。
4. 会话列表**单击**即可打开会话；再次单击 SSH 会再开一个标签，**串口会话始终只保留一个实例**。
