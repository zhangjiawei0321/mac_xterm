...
1. 左侧功能条点 **SSH** 或按 ⌘N → 填主机/端口，会话名自动为「主机:(用户名)」；用户名/密码**都可留空** → “保存后立即连接”。
2. 连接流程：
   - **用户名留空**：连接时先弹「需要登录用户名」，手动输入（不会默认用 Mac 用户名）；
   - **保存了密码**：`SSH_ASKPASS` 自动登录，直接进入 shell；
   - **密码留空**：终端内交互输入；登录成功后约 6 秒询问“是否保存该密码”，保存后下次自动登录；
   - **密码错误/连不上**：自动弹出“重输密码”，更新后自动重连。
3. `exit` 退出后标签标题加“（已断开）”，点 × 关闭。
4. 会话列表**单击**即可打开并自动聚焦终端；再次单击 SSH 会再开一个新标签，**串口始终单实例**。

## 串口使用（Serial 示例）

1. 左侧功能条点 **串口** → 选择设备（`/dev/cu.*`，可点刷新）、波特率等 → 确定。
   - 波特率支持 **921600** 与「自定义…」任意输入（macOS 按数值直接设置）。
2. 连接成功后终端里会有提示，直接收发数据。
3. 会话名会自动填为「设备名:(波特率)」，例如 `cu.usbserial-0001:(115200)`。
